...
impl Auth {
    ...
    pub async fn get_bearer_token(&self) -> Result<String> {
        let xbl_data = self.authenticate_with_xbl().await?;
        let xsts_token = self.authenticate_with_xsts(&xbl_data.token).await?;
    ...
    async fn authenticate_with_xbl(&self) -> Result<XBLData> {
    ...
    async fn authenticate_with_xsts(&self, token: &str) -> Result<String> {
